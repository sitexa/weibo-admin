<template>
  <div class="app-container">
    <!--查询  -->
    <el-row>
      <el-input v-model="tableQuery.nick" style="width:200px;" placeholder="昵称"/>
      <span style="margin-right: 15px;"/>
      <el-tooltip class="item" content="搜索" placement="top">
        <el-button v-perm="'b:user:query'" icon="el-icon-search" circle @click="fetchData(1)"/>
      </el-tooltip>
    </el-row>
    <div style="margin-bottom: 30px;"/>
    <el-button v-perm="'b:user:add'" type="primary" icon="el-icon-plus" size="mini" @click="handleCreate">
      {{ textMap.create }}
    </el-button>
    <div style="margin-bottom: 30px;"/>
    <!--列表-->
    <el-table
      ...
      <el-table-column prop="uname" label="登录名"/>
      ...
      <el-table-column prop="time" label="创建时间">
        <template slot-scope="scope">
          <span v-text="parseTime(scope.row.created,'{y}-{m}-{d}')"/>
        </template>
      </el-table-column>
      <el-table-column prop="time" label="更新时间">
        <template slot-scope="scope">
          <span v-text="parseTime(scope.row.updated,'{y}-{m}-{d}')"/>
        </template>
      </el-table-column>
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button-group>
            ...
            <el-tooltip v-if="!hasAdminRole(scope.row)" content="修改角色" placement="top">
              <el-button size="mini" type="warning" icon="el-icon-star-off" @click="handleUpdateUserRoles(scope.$index,scope.row)"/>
            </el-tooltip>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="30%">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="120px">
        ...
        <el-form-item v-if="dialogStatus=='create'" label="登录名" prop="uname">
        ...
        <el-form-item label="确认密码" prop="pwd2">
          <el-input v-model="temp.pwd2" type="password"/>
        </el-form-item>
    ...
    <!--弹出窗口：修改用户角色-->
    <el-dialog :visible.sync="editRolesDialogVisible" title="修改用户的角色" width="30%">
      <div>
        <el-checkbox :indeterminate="isIndeterminate" v-model="checkAll" @change="handleCheckAllChange">全选</el-checkbox>
        <div style="margin: 15px 0;"/>
        <el-checkbox-group v-model="updateUserRolesData.rids">
          <el-checkbox v-for="role in roleOptions" :label="role.id" :key="role.id" class="role-checkbox">
            {{ role.val }}
          </el-checkbox>
        </el-checkbox-group>
      </div>
